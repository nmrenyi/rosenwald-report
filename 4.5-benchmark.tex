...
To improve the annotation accuracy, we manually cropped the advertisements at the top and bottom of each page, and split the two columns into two images. Removing advertisements and splitting the columns could help the accuracy, as is discussed in Section~\ref{sec:image-preprocessing}.
...
As is shown in Figure~\ref{fig:four-distributions}, we visualised the distribution of field-level matching rate, character-level matching rate, number of different fields and number of total fields over the files in 210 columns. We can notice the long tail phenomenon in field-level matching rate, character-level matching rate and number of different fields. Annotators need particularly long time to solve the files in these long tails.

Limited by project time, for the 210 columns dataset, we only keep the files that meet the following criteria: with less than 70 different fields, with higher than 0.7 field matching rate and higher than 0.6 character matching rate.

With this filtering, we could maximize the utility of the machine consensus and save human labour, although there is the risk to favour the shorter columns and favour the files where MLLM works better. Fortunately the extraction pipeline we are going to use does not use MLLM, so it's less risky.
